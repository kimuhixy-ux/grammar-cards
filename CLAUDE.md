...
| ENGLISH EX Grammar & Usage | 540p | A5 | あり（ただしOCR崩れあり・信頼不可。`pdftotext`は使用不可） |

ENGLISH EX Grammar & Usageは前2冊と構成が異なり、「問題集」形式（前付けp1-15の後、
偶数ページ=番号付き設問／奇数ページ=「解答＆重要ポイント」の2ページ1組が本編p16以降で一貫）。
設問ページ単独では正解が読み取れないため、抽出時は設問＋解答の2ページを1回のVision API呼び出しに
まとめて渡し、正解を反映した完成形の英文をexample_sentencesとする（詳細は`pipeline/01_extract_pages.py`の
`BOOK_PAGE_GROUP`/`EXTRACTION_PROMPT_PAIRED`、および`進捗.md`のPhase 6セクション参照）。

→ **いずれもCanon複合機でスキャンされた画像PDFであり、通常のテキスト抽出は使えない。**
